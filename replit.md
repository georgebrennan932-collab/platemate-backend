...
- **Google OAuth Policy Compliance**: Fixed "disallowed_useragent" error by updating Capacitor Browser configuration to use fullscreen presentation style, ensuring proper Chrome Custom Tabs (Android) and SFSafariViewController (iOS) instead of popover WebViews that Google blocks
- **WebView Detection System**: Added embedded browser detection utility that identifies Instagram, Facebook, LinkedIn, Twitter, and other in-app browsers, with user-friendly warning banners guiding users to open in their default browser for secure Google authentication
- **Enhanced Step Rewards Feature**: Expanded suggestion database from 2 to 8-10 diverse options per calorie tier across 6 precise ranges (0-100, 100-200, 200-350, 350-500, 500-700, 700+), organized into categories (Healthy Options, Protein-Packed, Quick Snacks, Treats & Rewards) for better variety
- **Native Google Sign-In for Mobile**: Replaced browser-based OAuth flow with @capgo/capacitor-social-login plugin for true native Google authentication using platform SDKs (Google Sign-In SDK on Android/iOS)
- **Seamless Mobile Authentication**: Mobile users now authenticate directly through native Google Sign-In dialogs without leaving the app or encountering WebView restrictions
