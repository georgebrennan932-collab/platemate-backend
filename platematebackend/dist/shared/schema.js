...
export const foodAnalyses = pgTable("food_analyses", {
    ...
    detectedFoods: jsonb("detected_foods").notNull().$type(),
...
export const foodConfirmations = pgTable("food_confirmations", {
    ...
    suggestedFoods: jsonb("suggested_foods").notNull().$type(), // AI's food suggestions
    alternativeOptions: jsonb("alternative_options").$type(), // Other USDA options for manual selection
    status: varchar("status").notNull().default("pending"), // pending, confirmed, rejected
    finalFoods: jsonb("final_foods").$type(), // User-confirmed or modified foods
...
export const aiCoachMemory = pgTable("ai_coach_memory", {
    ...
    // Mood Tracking
    recentMoods: jsonb("recent_moods").$type().default([]),
