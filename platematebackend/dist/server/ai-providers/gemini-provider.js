...
export class GeminiProvider extends AIProvider {
    constructor() {
        super();
        this.name = "Gemini";
        this.priority = 1; // Highest priority (free, unlimited)
        this.maxRetries = 2;
...
Focus on accurate food identification and CONSERVATIVE portion estimation using any reference objects you can see. Be specific with portions (e.g., "100g", "1 medium slice", "150ml", "25g" for UK crisps). Only return the JSON object, no additional text.`,
            ...
            const totalCalories = mockNutritionFoods.reduce((sum, food) => sum + food.calories, 0);
            const totalProtein = mockNutritionFoods.reduce((sum, food) => sum + food.protein, 0);
            const totalCarbs = mockNutritionFoods.reduce((sum, food) => sum + food.carbs, 0);
            const totalFat = mockNutritionFoods.reduce((sum, food) => sum + food.fat, 0);
            const result = {
                imageUrl: imagePath,
            ...
            if (error?.message?.includes('quota') || error?.message?.includes('rate')) {
                providerError = this.createError('RATE_LIMIT', 'Gemini rate limit exceeded', true, true, 120 // Retry after 2 minutes
                );
            }
            else if (error?.message?.includes('GEMINI_API_KEY')) {
                providerError = this.createError('CONFIG_ERROR', 'Gemini API key not configured', false, false);
            }
            else {
                providerError = this.createError('ANALYSIS_ERROR', `Gemini analysis failed: ${error.message}`, false, true, 60);
            }
...
If you can't clearly identify something, don't include it. Be as accurate as possible with food identification.`,
            ...
            if (error?.message?.includes('quota') || error?.message?.includes('rate')) {
                ...
                providerError = this.createError('DETECTION_ERROR', `Gemini food detection failed: ${error.message}`, false, true, 60);
            }
            this.recordError(providerError);
            throw providerError;
        }
...
IMPORTANT: UK food names like "jacket potato" are SINGLE items - do NOT split them! Use realistic portions and accurate nutrition values. Only return the JSON object, no additional text.`;
            ...
            if (error?.message?.includes('quota') || error?.message?.includes('rate')) {
                ...
                providerError = this.createError('ANALYSIS_ERROR', `Gemini text analysis failed: ${error.message}`, false, false);
            }
            this.recordError(providerError);
            throw providerError;
        }
...
${JSON.stringify(analysisData, null, 2)}`;
            ...
            let providerError;
            if (error?.message?.includes('quota') || error?.message?.includes('rate')) {
                providerError = this.createError('RATE_LIMIT', 'Gemini rate limit exceeded', true, true, 120);
            }
            else if (error?.message?.includes('GEMINI_API_KEY')) {
                providerError = this.createError('CONFIG_ERROR', 'Gemini API key not configured', false, false);
            }
            else {
                providerError = this.createError('ADVICE_ERROR', `Gemini advice generation failed: ${error.message}`, false, true, 60);
            }
            this.recordError(providerError);
            throw providerError;
        }
...
- Keep responses focused and engaging (200-300 words max)`;
            ...
            if (error?.status === 503 || error?.message?.includes('UNAVAILABLE') || error?.message?.includes('overloaded')) {
                providerError = this.createError('SERVICE_UNAVAILABLE', 'Gemini is temporarily overloaded, retrying...', true, // retryable
                true, // use fallback
                ...
                providerError = this.createError('QUESTION_ERROR', `Gemini question answering failed: ${error.message}`, false, true, 60);
            }
            this.recordError(providerError);
            throw providerError;
        }
...
Respond with valid JSON object only.`;
            ...
            if (error?.message?.includes('quota') || error?.message?.includes('rate')) {
                ...
                providerError = this.createError('COACHING_ERROR', `Gemini coaching generation failed: ${error.message}`, false, true, 60);
            }
            this.recordError(providerError);
            throw providerError;
        }
...
Respond with JSON object containing 'tips' array.`;
            ...
            if (error?.message?.includes('quota') || error?.message?.includes('rate')) {
                ...
                providerError = this.createError('TIPS_ERROR', `Gemini tips generation failed: ${error.message}`, false, true, 60);
            }
            this.recordError(providerError);
            throw providerError;
        }
  ...
  "recipes": [
    {
      ...
      "dietaryInfo": ["Vegetarian", "High Protein"]
            ...
            if (error?.status === 429 || error?.code === 'RATE_LIMIT_EXCEEDED') {
                providerError = this.createError('RATE_LIMIT', 'Gemini rate limit exceeded', true, true, 60);
            }
            else if (error?.status >= 500) {
                providerError = this.createError('SERVER_ERROR', `Gemini server error: ${error.message}`, false, true, 30);
            }
            else {
                providerError = this.createError('RECIPE_ERROR', `Gemini recipe generation failed: ${error.message}`, false, false);
            }
            this.recordError(providerError);
            throw providerError;
        }
