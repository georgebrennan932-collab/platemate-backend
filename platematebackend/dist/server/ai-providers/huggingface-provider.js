...
export class HuggingFaceProvider extends AIProvider {
    constructor() {
        ...
        this.priority = 2; // Second priority (free backup)
        this.maxRetries = 2;
    ...
    async queryVisionModel(imageBytes) {
    ...
    async queryTextModel(prompt, maxTokens = 1000) {
        ...
        if (!response.ok) {
            ...
            if (response.status === 429 || errorText.includes('rate limit')) {
                ...
                throw this.createError('RATE_LIMIT', 'HuggingFace rate limit exceeded', true, true, retryAfter ? parseInt(retryAfter) : 60);
            }
...
Only return the JSON object, nothing else.`;
            ...
            if (error?.message?.includes('quota') || error?.message?.includes('rate') || error.isRateLimit) {
                providerError = this.createError('RATE_LIMIT', 'HuggingFace rate limit exceeded', true, true, error.retryAfter || 60);
            }
            else if (error?.message?.includes('API') || error?.message?.includes('network')) {
                providerError = this.createError('API_ERROR', error.message, false, true);
            }
            else {
                providerError = this.createError('UNKNOWN_ERROR', error.message, false, true);
            }
