...
export class AIManager {
     ...
     */
    async extractTextFromImage(imagePath, prompt) {
        ...
        for (const provider of availableProviders) {
            for (let attempt = 1; attempt <= provider.maxRetries; attempt++) {
                try {
                    ...
                    if (provider.name === 'gemini' && provider.model) {
     ...
     */
    async detectFoodNamesFromImage(imagePath) {
        const availableProviders = this.getAvailableProviders();
        // Try each available provider
        for (const provider of availableProviders) {
            for (let attempt = 1; attempt <= provider.maxRetries; attempt++) {
     ...
     */
    async getNutritionFromUSDA(foodNames) {
        try {
     ...
     */
    async tryOpenFoodFactsFallback(foodName) {
        try {
            ...
            if (results && results.length > 0 && results[0].nutrition_per_100g !== "not found") {
                const nutrition = results[0].nutrition_per_100g;
                return {
                    name: foodName,
                    nutrition: {
     ...
     */
    async combineDetectionWithNutrition(imagePath, detection, nutritionMap) {
        const detectedFoods = [];
        let totalCalories = 0;
        let totalProtein = 0;
        let totalCarbs = 0;
     ...
     */
    async combineAIPortionsWithUSDANutrition(imagePath, aiAnalysis, nutritionMap) {
        const detectedFoods = [];
        let totalCalories = 0;
        let totalProtein = 0;
        let totalCarbs = 0;
     ...
     */
    formatPortionText(portion, foodName, portionGrams) {
        ...
        if (isWholePotato) {
            ...
            let size = 'medium';
            if (gramsPerPotato < 175)
                size = 'small';
            else if (gramsPerPotato > 250)
                size = 'large';
            // Format with count
     ...
     */
    convertPortionToGrams(portion, foodName) {
        const portionLower = portion.toLowerCase().trim();
        const foodLower = foodName?.toLowerCase() || '';
        ...
        if (isEgg) {
            if (portionLower.includes('large'))
                return num * 50; // Large egg ≈ 50g
            if (portionLower.includes('medium'))
                return num * 44; // Medium egg ≈ 44g  
            if (portionLower.includes('small'))
                return num * 38; // Small egg ≈ 38g
        ...
        if (isPotato && !foodLower.includes('chip') && !foodLower.includes('fries') && !foodLower.includes('sweet')) {
            if (portionLower.includes('large'))
                return num * 300; // Large potato ≈ 300g
            if (portionLower.includes('medium'))
                return num * 200; // Medium potato ≈ 200g
            if (portionLower.includes('small'))
                return num * 150; // Small potato ≈ 150g
    ...
    getFoodIcon(foodName) {
        ...
        if (lowerName.includes('vegetable') || lowerName.includes('broccoli') || lowerName.includes('carrot'))
            return '🥕';
        if (lowerName.includes('salad') || lowerName.includes('lettuce'))
            return '🥗';
     ...
     */
    async legacyAnalyzeFoodImage(imagePath) {
        const availableProviders = this.getAvailableProviders();
        // Try each available provider
        for (const provider of availableProviders) {
            for (let attempt = 1; attempt <= provider.maxRetries; attempt++) {
     ...
     */
    async parseFoodNamesFromText(foodDescription) {
            ...
            if (matches.length > 1) {
                ...
                for (let i = 1; i < matches.length; i++) {
                    ...
                    if (match.index !== undefined) {
                        ...
                        const hasNonDescriptor = wordsBeforeArray.some(word => word.length > 2 && !portionDescriptors.test(word) && !/^\d+$/.test(word));
                        if (hasNonDescriptor) {
     ...
     */
    async combineTextWithNutrition(originalText, foodItems, nutritionMap) {
        const detectedFoods = [];
        let totalCalories = 0;
        let totalProtein = 0;
        let totalCarbs = 0;
        ...
        console.log(`📊 combineTextWithNutrition returning ${detectedFoods.length} foods:`, detectedFoods.map(f => `${f.name} (${f.calories} cal)`));
        return {
     ...
     */
    async legacyAnalyzeFoodText(foodDescription, clientTimeInfo) {
        const availableProviders = this.getAvailableProviders();
     ...
     */
    async answerNutritionQuestion(question, userEntries, userProfile, nutritionGoals, coachMemory, personality) {
        const availableProviders = this.getAvailableProviders();
        // Try each available provider
        for (const provider of availableProviders) {
            for (let attempt = 1; attempt <= provider.maxRetries; attempt++) {
                try {
                    const response = await provider.answerNutritionQuestion(question, userEntries, userProfile, nutritionGoals, coachMemory, personality);
                    return response;
                }
                catch (error) {
     ...
     */
    async answerNutritionQuestionWithContext(question, userContext, coachMemory, personality, clientTimeInfo) {
        const availableProviders = this.getAvailableProviders();
        ...
        for (const provider of availableProviders) {
            for (let attempt = 1; attempt <= provider.maxRetries; attempt++) {
                try {
                    ...
                    const response = await provider.answerNutritionQuestion(fullPrompt, userContext.recentDiary || [], userContext.profile || undefined, userContext.nutritionGoals || undefined, coachMemory, personality, clientTimeInfo);
                    return response;
                }
                catch (error) {
                    // If it's a rate limit error, try next provider immediately
        ...
        for (const provider of availableProviders) {
            for (let attempt = 1; attempt <= provider.maxRetries; attempt++) {
                try {
                    ...
                    if ('generateRecipes' in provider && typeof provider.generateRecipes === 'function') {
                        const result = await provider.generateRecipes(dietaryFilter, userProfile);
                        return result;
                    }
                    else {
     ...
     */
    getFallbackRecipes(dietaryFilter) {
        const baseRecipes = [
            {
                id: "fallback-1",
        ...
        return baseRecipes.filter(recipe => {
            ...
            return recipe.dietaryInfo.some(diet => diet.toLowerCase().includes(filterLower) ||
     ...
     */
    async analyzeMenu(menuText, nutritionGoals) {
        const availableProviders = this.getAvailableProviders();
...
Return ONLY a valid JSON object in this exact format:
{
  "recommendations": [
     ...
     */
    getFirstAvailableProvider(preferredNames) {
        for (const name of preferredNames) {
            const provider = this.providers.find(p => p.name.toLowerCase() === name.toLowerCase() && p.isAvailable());
            if (provider) {
                return provider;
            }
        }
