...
export class USDAService {
     ...
     */
    extractNutritionData(usdaFood, portionGrams = 100) {
        const nutrients = usdaFood.foodNutrients;
        // USDA nutrient IDs for key macronutrients
        const findNutrient = (numbers, label) => {
    ...
    async findBestMatch(foodName) {
        try {
            ...
            const scoredFoods = searchResults.foods.map(food => {
                ...
                const hasBasicNutrients = ['203', '204', '205', '208'].every(id => food.foodNutrients.some(n => n.nutrient && n.nutrient.number === id && n.amount > 0));
                ...
                if (searchName.includes('back bacon') || searchName.includes('bacon')) {
                    if (description.includes('pork') && description.includes('bacon'))
                        score += 150;
                    if (description.includes('chicken') || description.includes('turkey'))
                        score -= 200; // Avoid poultry for bacon
                }
                if (searchName.includes('black pudding')) {
                    if (description.includes('sausage') || description.includes('blood'))
                        score += 150;
                    if (description.includes('banana') || description.includes('chocolate'))
                        score -= 200; // Avoid desserts
                }
                ...
                if (description.split(',').length <= 2)
                    score += 30; // Prefer simple descriptions
                // Penalize processed foods
                if (description.includes('croissant') || description.includes('cake') || description.includes('cookie'))
                    score -= 100;
                if (description.includes('canned') || description.includes('frozen prepared'))
                    score -= 20;
                if (description.includes('with ') || description.includes('sauce') || description.includes('dressing'))
                    score -= 15;
                ...
                if (description.startsWith(foodName.toLowerCase()))
                    score += 200;
                // Boost bacon-related foods when searching for bacon
                if (searchName.includes('bacon')) {
                    if (description.includes('bacon') && description.includes('pork'))
                        score += 100;
                    if (description.includes('cured') && description.includes('pork'))
                        score += 75;
                }
                return { food, score };
            });
     ...
     */
    async findMultipleMatches(foodNames) {
