...
export class ShiftMealPlanService {
    ...
    async generateWeeklyMealPlan(userId, shifts, userProfile) {
        if (shifts.length === 0) {
    ...
    buildSystemPrompt(userProfile, dailyCalorieTarget) {
        let profileContext = "";
        if (userProfile) {
            profileContext = `
User Profile:
...
Return a structured JSON response following the schema exactly.`;
    ...
    buildUserPrompt(shifts, userProfile, dailyCalorieTarget) {
...
Focus on realistic, affordable meals using ingredients commonly available in UK supermarkets. Prioritize meal prep efficiency and portability.`;
    ...
    async generateWithGemini(provider, systemPrompt, userPrompt, shifts) {
    ...
    async generateWithOpenAI(provider, systemPrompt, userPrompt, shifts) {
        const client = provider.client;
...
export const shiftMealPlanService = new ShiftMealPlanService({} // Will be injected by routes
);
