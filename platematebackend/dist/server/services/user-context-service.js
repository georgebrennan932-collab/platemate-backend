...
class UserContextService {
    ...
    async getUserContext(userId) {
        try {
            ...
            const [profile, nutritionGoals, recentDiary, recentWeights, recentDrinks, recentWaterIntake, shoppingList, stepsData, challengeData, shiftSchedules,] = await Promise.all([
...
- Total Points: ${context.totalPoints}`);
        ...
        if (context.shoppingList && context.shoppingList.length > 0) {
            const uncheckedItems = context.shoppingList.filter((item) => !item.checked || item.checked === 0);
            const checkedItems = context.shoppingList.filter((item) => item.checked === 1);
  ...
  ${itemsList}`);
    ...
    // Helper methods
    async enrichDiaryEntries(entries) {
    ...
    async getShoppingList(userId) {
        try {
            ...
            if (typeof storage.getShoppingList === 'function') {
                return await storage.getShoppingList(userId);
            }
            return [];
        }
        catch {
    ...
    async getStepsToday(userId) {
        try {
            if (typeof storage.getStepsToday === 'function') {
                return await storage.getStepsToday(userId);
            }
            return { stepCount: 0 };
        }
        catch {
    ...
    async getChallengeData(userId) {
        try {
            ...
            let active = [];
            let points = 0;
            if (typeof storage.getActiveChallenges === 'function') {
                active = await storage.getActiveChallenges(userId);
            }
            if (typeof storage.getUserPoints === 'function') {
                const pointsData = await storage.getUserPoints(userId);
    ...
    calculateWeeklyMealCount(diaryEntries) {
    ...
    calculateAverageDailyCalories(diaryEntries) {
        const oneWeekAgo = new Date();
        oneWeekAgo.setDate(oneWeekAgo.getDate() - 7);
        ...
        const uniqueDays = new Set(recentEntries.map(e => new Date(e.mealDate).toDateString())).size;
